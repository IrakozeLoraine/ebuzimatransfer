...
import type { LoginRequest, SetPasswordRequest, TokenResponse, UserMe, UpdateProfilePayload } from "@/types/auth";
...
export const updateProfile = async (payload: UpdateProfilePayload): Promise<UserMe> => {
  const { data } = await api.put<UserMe>("/auth/me", payload);
  return data;
};
